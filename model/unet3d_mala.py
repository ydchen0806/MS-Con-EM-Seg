...
class UNet3D_MALA(nn.Module):
	def __init__(self, output_nc=1, if_sigmoid=True, init_mode='kaiming', show_feature=False):
		...
		self.conv10 = nn.Conv3d(300, 300, 3, stride=1, padding=0, dilation=1, groups=1, bias=True)
		...
		self.conv13 = nn.Conv3d(60, 60, 3, stride=1, padding=0, dilation=1, groups=1, bias=True)
		...
		self.conv16 = nn.Conv3d(12, 12, 3, stride=1, padding=0, dilation=1, groups=1, bias=True)
	...
	def crop_and_concat(self, upsampled, bypass, crop=False):
		...
		return upsampled + bypass # torch.cat((upsampled, bypass), 1)
	...
	def forward(self, input):
		...
		# print('shape of conv9: ', conv9.shape, 'shape of conv6: ', conv6.shape)
		mc1 = self.crop_and_concat(conv9, conv6, crop=True)
		# print('shape of mc1: ', mc1.shape)
...
class UNet3D_MALA_ns(nn.Module):
	def __init__(self, output_nc=1, if_sigmoid=True, init_mode='kaiming', show_feature=False):
		super(UNet3D_MALA_ns, self).__init__()
		self.if_sigmoid = if_sigmoid
		self.init_mode = init_mode
		self.show_feature = show_feature
	...
	def forward(self, input):
		...
		if self.if_sigmoid:
			output = torch.sigmoid(output)
		if self.show_feature:
			return mc1,mc2,[conv8, conv11, conv14, conv17], output
		else:
			return output
...
if __name__ == '__main__':
	...
	model = UNet3D_MALA(output_nc=1, if_sigmoid=True,show_feature=False).to('cuda:0')
	# 参数量统计
