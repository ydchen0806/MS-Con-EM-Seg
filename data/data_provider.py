...
import sys
sys.path.append('/braindat/lab/chenyd/code/UNETR/code/SuperHuman-main/scripts/scripts')
...
class Train(Dataset):
	def __init__(self, cfg):
		...
		self.data_folder = '/braindat/lab/chenyd/DATASET/unlabel_data'
		self.train_datasets = sorted(glob(os.path.join(self.data_folder, '*hdf')))
		...
		for k in range(len(self.train_datasets)):
			...
			# load raw data
			f_raw = h5py.File(self.train_datasets, 'r')
			data = f_raw['main'][:]
			f_raw.close()
...
if __name__ == '__main__':
	...
	with open('/root/code/SuperHuman-main/scripts/scripts/config/' + cfg_file, 'r') as f:
	...
	if cfg.DATA.shift_channels is not None:
		...
		cfg.shift = None
	out_path = os.path.join('/root/code/SuperHuman-main', 'seg_data_temp')
