...
def init_project(cfg):
    ...
    prefix = cfg.time
    model_name = cfg.time + '_' + cfg.MODEL.model_type + '_pretrain_' + str(cfg.TRAIN.pretrain) + '_' \
    ...
    # 合并成新的路径
    path = '/'.join(path_list)
    path = '/braindat/lab/chenyd/MODEL/NeurIPS_0521_SwinUnetr_EM_02'
...
def build_model(cfg, writer):
    ...
    if cfg.MODEL.model_type == 'mala':
        ...
        model = UNETR(
                        ...
                        show_feature=False,
                        dropout_rate=0.1)
    elif cfg.MODEL.model_type == 'SwinUNETR':
        print('load SwinUNETR model!')
        model = SwinUNETR(args, img_size = (32,160,160),in_channels=cfg.MODEL.input_nc, out_channels=cfg.MODEL.output_nc, depths=(2,4,2,2), feature_size=48)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-p', '--pretrained', type=bool, default=True, help='path to config file')
    parser.add_argument('-w','--pretrained_path', type=str, default='/braindat/lab/chenyd/code_230508/Neurips23_imgSSL_swin/model_swinvit.pt', help='path to config file')
    # parser.add_argument('--dataset_name', type=str, default='cremi-B', help='path to config file')
    # parser.add_argument('--train_split', type=int, default=10, help='path to config file')
